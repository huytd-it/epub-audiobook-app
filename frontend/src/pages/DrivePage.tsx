...
export function DrivePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {activeSection === "exports" && <Card><CardHeader><CardTitle className="text-sm">Lịch sử bản export</CardTitle></CardHeader><CardContent className="p-0">{exports.length === 0 ? <EmptyState text="Chưa có bản export nào." /> : <div className="divide-y divide-border">{exports.map((item) => <div key={item.id} className="p-4 flex justify-between gap-3 text-xs"><div><div className="font-semibold">Export #{item.id} · Patch #{item.patch_id}</div><div className="font-mono text-muted-foreground break-all">{item.export_dir}</div><div className="text-[10px] text-muted-foreground">{new Date(item.created_at).toLocaleString("vi-VN")}</div></div><Button variant="outline" size="sm" asChild><a href={item.export_dir}><ExternalLink className="h-3.5 w-3.5" /> Mở</a></Button></div>)}</div>}</CardContent></Card>}
