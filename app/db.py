...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    # Flow (luồng tuỳ biến) đã bị gỡ. Thứ tự bắt buộc: bỏ index trước, vì SQLite từ chối
    # DROP COLUMN khi cột còn nằm trong index; rồi mới drop bảng, vì job.flow_run_id giữ
    # FK tới flow_run.
    conn.execute("DROP INDEX IF EXISTS idx_job_flow_run")
    for name in ("flow_run_id", "node_id"):
        if name in job_existing:
            conn.execute(f"ALTER TABLE job DROP COLUMN {name}")
    conn.execute("DROP TABLE IF EXISTS flow_run")
    conn.execute("DROP TABLE IF EXISTS flow_definition")
    conn.execute(
